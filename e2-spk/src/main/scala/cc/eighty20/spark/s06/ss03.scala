...
object ss03 {
  def main(args: Array[String]) {
    if (args.length < 3) {
      System.err.println("""
        |Usage: ss03 <brokers> <topics> <numOfTop>
    ...
    val sparkConf = new SparkConf().setAppName("ss03").setMaster(masterURL)
    ...
    freq.foreachRDD{(rdd, time) =>
      val top = rdd.map(_.swap).top(topN)
