...
public class S05_02_Heartbeat_Subscriber {	
	...
	public static void main(String[] args) throws Exception {
		...
		props.put("enable.auto.commit","true"); // 當設定為"true"的時候, Consumer的Offset的commit會定期的執行 (預設為: true)
	    props.put("auto.commit.interval.ms","10000"); //如果"enable.auto.commit"設為"true"時, 設定多久client會去commit所consumer的offset數字(單位為: millis)	    
	...
	private static CommandLine process_args(String[] args){
		...
		try {
		    ...
		    if(validate_args(cmdLine)){
				...
				formatter.printHelp("S05_02_Heartbeat_Subscriber", options );
				System.exit(-1);
			}
		}
