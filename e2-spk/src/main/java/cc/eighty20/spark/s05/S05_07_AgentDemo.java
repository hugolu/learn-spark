...
public class S05_07_AgentDemo {
	...
	private static void login(Socket socket, String userid) throws Exception{
		// { type: 'users', op: 'update', data: { socketid: socket.id, user_profile: { userid: userid, gender: user_gender }}
		
		JSONObject user_profile = new JSONObject();
		user_profile.put("userid", userid);
		user_profile.put("gender", "x");
		
		JSONObject data = new JSONObject();
		data.put("socketid", socket.id());
		data.put("user_profile", user_profile);
		...
		// 送login訊息回到Socket.IO
		socket.emit("login", msg.toString(), new Ack(){
			@Override
			public void call(Object... args) {	
