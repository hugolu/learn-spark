...
public class S05_05_Heartbeat_Publisher_WithoutKey {	
	...
	public static void main(String[] args) throws Exception {
		...
		Integer msgs_per_second = Integer.parseInt(cmdLine.getOptionValue("n", "0")); // 設定每秒產生多少筆Event, 如果是"0"或null, 就是沒有限制
		...
		// 送出資料給Kafka
		 try {
			 while(true) {
				 ...
				 ProducerRecord<byte[], byte[]> message = new ProducerRecord<>(topic, null, eventPayload);
	...
	private static CommandLine process_args(String[] args){
		...
		try {
		    ...
		    if(validate_args(cmdLine)){
				...
				formatter.printHelp("S05_05_Heartbeat_Publisher_WithoutKey", options );
				System.exit(-1);
			}
		}
